...
void guid_pt_read_guid_pt_header(struct device* dev, struct guid_pt_header* header) {
    ...
    blockutil_read(dev, GUID_PT_HEADER_LBA, buffer);
...
uint8_t guid_pt_init(struct device* dev) {
    ...
    guid_pt_read_guid_pt_header(device_data->block_device, &header);
...
void guid_pt_read_guid_pt_entry(struct device* dev, struct guid_pt_entry* entry, uint8_t partition) {
    ...
    */
    uint64_t sector_size = blockutil_get_sector_size(device_data->block_device);
    ...
    */
    ...
    blockutil_read(device_data->block_device, header.gpt_array_lba + sector, data);
...
struct device* guid_pt_attach(struct device* block_device) {
     ...
     * device data
     */
    struct guid_pt_devicedata* device_data = (struct guid_pt_devicedata*)kmalloc(sizeof(struct guid_pt_devicedata));
