...
void sfs_read_superblock(struct device* dev, struct sfs_superblock* superblock) {
    ASSERT_NOT_NULL(dev);
    blockutil_read_sectors(dev, (uint8_t*)superblock, sizeof(struct sfs_superblock), 0);
}
...
void sfs_format(struct device* dev) {
    ...
    // write superblock
    blockutil_write_sectors(device_data->partition_device, (uint8_t*)&superblock, sizeof(struct sfs_superblock), 0);
}
